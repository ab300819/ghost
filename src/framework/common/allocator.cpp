...
ghost::Allocator::Allocator(size_t data_size, size_t page_size,
							size_t alignment)
	: m_pPageList(nullptr), m_pFreeList(nullptr) {
	Reset(data_size, page_size, alignment);
}
ghost::Allocator::~Allocator() {
	FreeAll();
}
void ghost::Allocator::Reset(size_t data_size, size_t page_size,
							 size_t alignment) {
	FreeAll();
...
void ghost::Allocator::Free(void* p) {
...
#if defined(_DEBUG)
void ghost::Allocator::FillFreePage(PageHeader* pPage) {
	// page header
	pPage->pNext = nullptr;
...
void ghost::Allocator::FillFreeBlock(BlockHeader* pBlock) {
	// block header + data
...
void ghost::Allocator::FillAllocatedBlock(BlockHeader* pBlock) {
	// block header + data
...
#endif
ghost::BlockHeader* ghost::Allocator::NextBlock(ghost::BlockHeader* pBlock) {
